...
namespace SILDMS.Service.Owner
{
    public class OwnerService : IOwnerService
    {
        ...
        public ValidationResult GetOwnerAddress(string id, string action, out List<Model.DocScanningModule.DSM_OwnerAddress> ownersList)
        {
            ownersList = _ownerDataService.GetOwnerAddress(id, action, out _errorNumber);
            if (_errorNumber.Length > 0)
            {
                return new ValidationResult(_errorNumber, _localizationService.GetResource(_errorNumber));
            }
