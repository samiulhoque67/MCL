...
namespace SILDMS.Web.UI.Areas.SecurityModule.Controllers
{
    ...
    public class UserController : Controller
    {
       ...
       [Authorize]
        ...
        [HttpGet]
        [Authorize]
        public async Task<dynamic> GetUserSort(string userId, string ownerID)
        {
            List<SEC_User> obUser = null;
            await Task.Run(() => _userService.GetAllUser("", "", out obUser));
            var result = obUser.Select(x => new
            {
                UserID = x.UserID,
                LevelName = x.LevelName,
                Supervisor = x.Supervisor,
                OwnerName = x.OwnerID + "-" + x.OwnerName,
                UserFullName = x.EmployeeID + "-" + x.UserFullName,
                RoleTitle = x.RoleTitle,
                RoleID = x.RoleID,
                UserName = x.UserName
        ...
        public async Task<dynamic> GetServers(string _OwnerID)
        {
            ...
            var result = from t in obServer where t.OwnerID == _OwnerID && t.Status==1
                    select new 
                    {
        ...
        //#endregion
         ...
         [HttpGet]
         [Authorize]
         public async Task<dynamic> GetSupervisor(string userId, string ownerID)
         {
             var obUser = new List<SEC_User>();
             await Task.Run(() => _userService.GetSupervisor(UserID, "", out obUser));
