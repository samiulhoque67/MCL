...
namespace SILDMS.Web.UI.Areas.DocScanningModule.Controllers
{
    public class OwnerProperIdentityController : Controller
    {
        ...
        [HttpGet]
        [Authorize]
        public async Task<dynamic> GetJobLocation()
        {
            List<SILDMS.Model.CBPSModule.Sys_MasterData> objMasterDatas = null;
            await Task.Run(() => _ownerLevelService.GetJobLocation(UserID, out objMasterDatas));

            var result = (from ol in objMasterDatas                          
                          select new SILDMS.Model.CBPSModule.Sys_MasterData
                          {
                              //MasterDataID = ol.MasterDataID,
                              //MasterDataValue = ol.MasterDataValue
                          }).ToList();
