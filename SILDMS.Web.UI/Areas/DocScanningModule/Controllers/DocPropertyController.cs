...
namespace SILDMS.Web.UI.Areas.DocScanningModule.Controllers
{
    ...
    public class DocPropertyController : Controller
    {
        ...
        [HttpGet]
        [Authorize]
        public async Task<dynamic> GetDocuments(string DocCategoryID = "", string OwnerLevelID = "", string OwnerID = "", string DocTypeID = "")
        {
            if (!string.IsNullOrEmpty(DocCategoryID) && !string.IsNullOrEmpty(OwnerLevelID) && !string.IsNullOrEmpty(OwnerID) && !string.IsNullOrEmpty(DocTypeID))
            {
                List<DSM_DocProperty> obDocProperty = null;
                await Task.Run(() => _docPropertyService.GetDocProperty("", UserID, out obDocProperty));
                var result = obDocProperty.Where(ob => (ob.DocCategoryID == DocCategoryID.Trim()) && (ob.OwnerLevelID == OwnerLevelID.Trim()) && (ob.OwnerID == OwnerID.Trim()) && (ob.DocTypeID == DocTypeID.Trim())).Select(x => new DSM_DocProperty
                {
                    DocPropertyID = x.DocPropertyID,
                    DocCategoryID = x.DocCategoryID,
                    OwnerLevelID = x.OwnerLevelID,
