...
{
    public class ClientSettlementDataService : IClientSettlementDataService
    {
        ...
        public List<OBS_ClientSettlement> GetShowClientReqList()
        {
            string errorNumber = string.Empty;
            List<OBS_ClientSettlement> ClientInfoList = new List<OBS_ClientSettlement>();
            DatabaseProviderFactory factory = new DatabaseProviderFactory();
            SqlDatabase db = factory.CreateDefault() as SqlDatabase;
            using (DbCommand dbCommandWrapper = db.GetStoredProcCommand("OBS_GetClientFinalPaymentRcvdList"))
            {
                // Execute SP. 
                DataSet ds = db.ExecuteDataSet(dbCommandWrapper);
                if (ds.Tables[0].Rows.Count > 0)
                {
                    DataTable dt1 = ds.Tables[0];
                    ClientInfoList = dt1.AsEnumerable().Select(reader => new OBS_ClientSettlement
                    {
                        ClientFinalPaymentRcvdID = reader.GetString("ClientFinalPaymentRcvdID"),
