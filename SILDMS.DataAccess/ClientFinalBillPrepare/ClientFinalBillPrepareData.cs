...
{
    public class ClientFinalBillPrepareData : IClientFinalBillPrepareData
    {
            ...
            using (DbCommand dbCommandWrapper = db.GetStoredProcCommand("OBS_GetClientBillRecvdList"))
            {
                ...
                if (ds.Tables[0].Rows.Count > 0)
                {
                    ...
                    vendorBillRecvdList = dt1.AsEnumerable().Select(reader => new VendorBillRecvd
                    {
                        ...
                        VendorQutnNo =  reader.GetString("ClientQuotationID"),
                        QuotationDate = reader.GetDateTime("AprvDate").ToString("dd/MM/yyyy"),
                        ...
                        QuotatDate = reader.GetDateTime("AprvDate").ToString("dd/MM/yyyy"),
                        ...
                        BillAmount= reader.GetToDecimal("BillAmount")
                    }).ToList();
                }
            }
        ...
        public List<OBS_VendorQutn> GetShowClientReqList()
        {
            string errorNumber = string.Empty;
            List<OBS_VendorQutn> ClientInfoList = new List<OBS_VendorQutn>();
            DatabaseProviderFactory factory = new DatabaseProviderFactory();
            SqlDatabase db = factory.CreateDefault() as SqlDatabase;
            using (DbCommand dbCommandWrapper = db.GetStoredProcCommand("OBS_GetClientWorkOrderList"))
            {
                // Execute SP. 
                DataSet ds = db.ExecuteDataSet(dbCommandWrapper);
                if (ds.Tables[0].Rows.Count > 0)
                {
                    ...
                    ClientInfoList = dt1.AsEnumerable().Select(reader => new OBS_VendorQutn
                    {
                        ...
                        RequisitionNo = reader.GetString("ClientReqID"),
                        RequisitionDate = reader.GetString("RequisitionDate"),
                        Address = reader.GetString("Address"),
                        VendorQutnNo = reader.GetString("ClientQutnAprvID"),
                        QuotationDate = reader.GetDateTime("AprvDate").ToString("dd/MM/yyyy"),
        ...
        public string SaveClientFinalBill(VendorBillRecvd billRecv)
        {
            ...
            try
            {
                ...
                if ((billRecv.ClientFinalBilPreprID) == 0)
                    billRecv.Action = "add";
                else
                    billRecv.Action = "edit";

                using (DbCommand dbCommandWrapper = db.GetStoredProcCommand("OBS_SetClientFinalBill"))
                {
                    // Set parameters 
