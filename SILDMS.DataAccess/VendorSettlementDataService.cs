...
{
    public class VendorSettlementDataService : IVendorSettlementDataService
    {
        ...
        public List<OBS_VendorSettlement> GetShowClientReqList()
        {
            string errorNumber = string.Empty;
            List<OBS_VendorSettlement> ClientInfoList = new List<OBS_VendorSettlement>();
            DatabaseProviderFactory factory = new DatabaseProviderFactory();
            SqlDatabase db = factory.CreateDefault() as SqlDatabase;
            using (DbCommand dbCommandWrapper = db.GetStoredProcCommand("OBS_GetVendorFinalPaymentList"))
            {
                // Execute SP. 
                DataSet ds = db.ExecuteDataSet(dbCommandWrapper);
                if (ds.Tables[0].Rows.Count > 0)
