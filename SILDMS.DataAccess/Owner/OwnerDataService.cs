    ...
    public class OwnerDataService : IOwnerDataService
    {
        ...
        public List<DSM_Owner> GetOwner(string id, string action, out string errorNumber)
        {
            ...
            using (DbCommand dbCommandWrapper = db.GetStoredProcCommand("GetOwner"))
            {
                ...
                db.AddInParameter(dbCommandWrapper, "@OwnerID", SqlDbType.VarChar, id == null ? "" : id);
        ...
        public List<DSM_OwnerAddress> GetOwnerAddress(string id, string action, out string errorNumber)
        {
            errorNumber = string.Empty;
            List<DSM_OwnerAddress> ownersList = new List<DSM_OwnerAddress>();
            DatabaseProviderFactory factory = new DatabaseProviderFactory();
            SqlDatabase db = factory.CreateDefault() as SqlDatabase;
            using (DbCommand dbCommandWrapper = db.GetStoredProcCommand("GetOwnerAddress"))
            {
                db.AddInParameter(dbCommandWrapper, "@UserID", SqlDbType.VarChar, action);
                db.AddInParameter(dbCommandWrapper, "@OwnerID", SqlDbType.VarChar, id == null ? "" : id);
                db.AddOutParameter(dbCommandWrapper, spErrorParam, DbType.Int32, 10);
                ...
                else
                {
                    ...
                    {
                        DataTable dt1 = ds.Tables[0];
                        ownersList = dt1.AsEnumerable().Select(reader => new DSM_OwnerAddress
                        {
                            OwnerAddressID = reader.GetString("OwnerAddressID"),
                            OwnerID = reader.GetString("OwnerID"),
                            OwnerAddress1 = reader.GetString("OwnerAddress1"),
                            OwnerAddress2 = reader.GetString("OwnerAddress2"),
                            OwnerCountry = reader.GetString("OwnerCountry"),
                            OwnerPhoneNo = reader.GetString("OwnerPhoneNo"),
                            OwnerMobileNo = reader.GetString("OwnerMobileNo"),
                            OwnerFax = reader.GetString("OwnerFax"),
                            OwnerEmail = reader.GetString("OwnerEmail"),

                            SkypeID = reader.GetString("SkypeID"),
                            ContactPerson = reader.GetString("ContactPerson"),
                            ContactCellNo = reader.GetString("ContactCellNo"),

                            OwnerTaxNo = reader.GetString("OwnerTaxNo"),
                            OwnerTINNo = reader.GetString("OwnerTINNo"),
                            OwnerBINNo = reader.GetString("OwnerBINNo")
