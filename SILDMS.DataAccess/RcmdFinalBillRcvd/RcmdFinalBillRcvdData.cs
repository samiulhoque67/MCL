...
{
    public class RcmdFinalBillRcvdData : IRcmdFinalBillRcvdData
    {
        ...
        public List<VendorBillRecvd> GetPOSearchList()
        {
            ...
            using (DbCommand dbCommandWrapper = db.GetStoredProcCommand("OBS_GetVendorRcmBillRecvdList"))
            {
                // Execute SP. 
        ...
        public List<VendorBillRecvd> GetShowVendorReqList()
        {
            ...
            DatabaseProviderFactory factory = new DatabaseProviderFactory();
            SqlDatabase db = factory.CreateDefault() as SqlDatabase;
            using (DbCommand dbCommandWrapper = db.GetStoredProcCommand("OBS_GetShowRcmPoList"))
            {
                // Execute SP. 
                DataSet ds = db.ExecuteDataSet(dbCommandWrapper);
                if (ds.Tables[0].Rows.Count > 0)
                {
                    ...
                    vendorBillRecvdList = dt1.AsEnumerable().Select(reader => new VendorBillRecvd
                    {
                        ...
                        WOInfoID = reader.IsNull("WOInfoID") ? string.Empty : reader.GetString("WOInfoID"),
                        AdvancePaidID = reader.IsNull("VendrAdvncPaymntID") ? string.Empty : reader.GetString("VendrAdvncPaymntID"),
                        RAWO = reader.GetToDecimal("AdvancClaimAprvAmt"),
                        POInstallmentNo = reader.GetInt32("POInstallmentNo"),
                        POInstallmentID = reader.GetInt32("POInstallmentID"),
                        POInstallmentAmt = reader.GetDouble("POInstallmentAmt"),
                        BillType = reader.GetString("BillType"),
        ...
        public string SaveVendorFinalBill(VendorBillRecvd billRecv)
        {
            ...
            try
            {
                ...
                if ((billRecv.VendorFinalBilRecmID) == 0)
                    billRecv.Action = "add";
                else
                    billRecv.Action = "edit";

                using (DbCommand dbCommandWrapper = db.GetStoredProcCommand("OBS_SetVendorRcmFinalBill"))
                {
                    // Set parameters 
